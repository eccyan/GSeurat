#!/usr/bin/env python3
"""
Debug: reproduce the exact pipeline workflow vs the experiment workflow.
Tests whether OpenPose is actually controlling composition.
"""
...
def render_openpose_png(keypoints, width=512, height=512):
    """Minimal OpenPose skeleton PNG renderer."""
    import struct, zlib
...
# Run right frame 0 pose (15 keypoints: nose, neck, r_shoulder, r_elbow, r_wrist,
# l_shoulder, l_elbow, l_wrist, mid_hip, r_hip, r_knee, r_ankle, l_hip, l_knee, l_ankle)
POSE_15KP = [
...
def build_workflow(concept_name, pose_name, seed, use_vae=False):
    """Build the same workflow as buildIPAdapterPoseWorkflow in comfyui.ts"""
    nodes = {
        ...
        "9": {"class_type": "SaveImage", "inputs": {"filename_prefix": "ipa_debug", "images": ["8", 0]}},
    }
    if use_vae:
        nodes["60"] = {"class_type": "VAELoader", "inputs": {"vae_name": "vae-ft-mse-840000-ema-pruned.safetensors"}}
        nodes["8"]["inputs"]["vae"] = ["60", 0]
    return nodes

def main():
    print("Uploading concept_right.png directly (no RemBG)...")
    concept_name = upload_image(CONCEPT_IMAGE)
    print(f"  → {concept_name}")

    print("Rendering pose skeleton (15 keypoints)...")
    pose_png = render_openpose_png(POSE_15KP)
    pose_name = upload_bytes(pose_png, "debug_pose_f0.png")
    ...
    # Test 1: No external VAE (same as round 2 experiment)
    print("\n--- Test 1: No external VAE (experiment-identical) ---")
    workflow = build_workflow(concept_name, pose_name, SEED, use_vae=False)
    try:
        ...
        for nid, out in entry["outputs"].items():
            if "images" in out:
                for img in out["images"]:
                    download_image(img, os.path.join(OUTPUT_DIR, "test1_no_vae.png"))
                    print("  Saved: test1_no_vae.png")
    except Exception as e:
        print(f"  ERROR: {e}")

    # Test 2: With external VAE
    print("\n--- Test 2: With external VAE ---")
    workflow = build_workflow(concept_name, pose_name, SEED, use_vae=True)
    try:
        pid = submit_workflow(workflow)
        entry = poll_completion(pid)
        for nid, out in entry["outputs"].items():
            if "images" in out:
                for img in out["images"]:
                    download_image(img, os.path.join(OUTPUT_DIR, "test2_with_vae.png"))
                    print("  Saved: test2_with_vae.png")
    except Exception as e:
        print(f"  ERROR: {e}")

    # Test 3: With RemBG on concept image first (simulates pipeline preRemBg)
    print("\n--- Test 3: With RemBG pre-processing (simulates pipeline) ---")
    rmbg_workflow = {
        "1": {"class_type": "LoadImage", "inputs": {"image": concept_name}},
        "2": {"class_type": "BRIA_RMBG_Zho", "inputs": {"image": ["1", 0]}},
        "3": {"class_type": "SaveImage", "inputs": {"filename_prefix": "rmbg_concept", "images": ["2", 0]}},
    }
    try:
        pid = submit_workflow(rmbg_workflow)
        entry = poll_completion(pid)
        rmbg_image_name = None
        for nid, out in entry["outputs"].items():
            if "images" in out:
                for img in out["images"]:
                    download_image(img, os.path.join(OUTPUT_DIR, "test3_rmbg_concept.png"))
                    rmbg_image_name = img["filename"]
                    print(f"  RemBG concept saved: test3_rmbg_concept.png ({rmbg_image_name})")

        if rmbg_image_name:
            # Re-upload the RemBG result as input
            rmbg_path = os.path.join(OUTPUT_DIR, "test3_rmbg_concept.png")
            rmbg_uploaded = upload_image(rmbg_path)
            print(f"  Re-uploaded RemBG result: {rmbg_uploaded}")

            workflow = build_workflow(rmbg_uploaded, pose_name, SEED, use_vae=False)
            pid = submit_workflow(workflow)
            entry = poll_completion(pid)
            for nid, out in entry["outputs"].items():
                if "images" in out:
                    for img in out["images"]:
                        download_image(img, os.path.join(OUTPUT_DIR, "test3_rmbg_ipa.png"))
                        print("  Saved: test3_rmbg_ipa.png")
    except Exception as e:
        print(f"  ERROR: {e}")
