#!/usr/bin/env python3
"""
Debug round 2: fix pose background to solid black (opaque).
"""
...
def render_openpose_png(keypoints, width=512, height=512):
    """Render OpenPose skeleton on SOLID BLACK (opaque) background."""
    pixels = bytearray(width * height * 4)
    # Fill with solid opaque black (R=0, G=0, B=0, A=255)
...
def build_workflow(concept_name, pose_name, seed, use_vae=False):
    nodes = {
        ...
        "9": {"class_type": "SaveImage", "inputs": {"filename_prefix": "ipa_debug2", "images": ["8", 0]}},
    }
    if use_vae:
...
def main():
    ...
    print("Rendering pose with OPAQUE black background...")
    pose_png = render_openpose_png(POSE_15KP)
    pose_name = upload_bytes(pose_png, "debug2_pose_f0.png")
    with open(os.path.join(OUTPUT_DIR, "pose_f0_opaque.png"), "wb") as f:
        f.write(pose_png)

    SEED = 42

    # Test A: Opaque black bg pose, no VAE
    print("\n--- Test A: Opaque black bg, no VAE ---")
        ...
        for nid, out in entry["outputs"].items():
            if "images" in out:
                for img in out["images"]:
                    download_image(img, os.path.join(OUTPUT_DIR, "testA_opaque_no_vae.png"))
                    print("  Saved: testA_opaque_no_vae.png")
    except Exception as e:
        print(f"  ERROR: {e}")

    # Test B: Opaque black bg pose, with VAE
    print("\n--- Test B: Opaque black bg, with VAE ---")
    ...
    try:
        ...
        for nid, out in entry["outputs"].items():
            if "images" in out:
                for img in out["images"]:
                    download_image(img, os.path.join(OUTPUT_DIR, "testB_opaque_with_vae.png"))
                    print("  Saved: testB_opaque_with_vae.png")
    except Exception as e:
        print(f"  ERROR: {e}")

    # Test C: Just txt2img (no IP-Adapter, no ControlNet) to verify basic generation
    print("\n--- Test C: Basic txt2img (no IPA, no CN) ---")
    basic = {
        "4": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": "AnythingV5_v5PrtRE.safetensors"}},
        "6": {"class_type": "CLIPTextEncode", "inputs": {"text": PROMPT, "clip": ["4", 1]}},
        "7": {"class_type": "CLIPTextEncode", "inputs": {"text": NEGATIVE, "clip": ["4", 1]}},
        ...
        "8": {"class_type": "VAEDecode", "inputs": {"samples": ["3", 0], "vae": ["4", 2]}},
        "9": {"class_type": "SaveImage", "inputs": {"filename_prefix": "basic_test", "images": ["8", 0]}},
    }
    try:
        pid = submit_workflow(basic)
        entry = poll_completion(pid)
        for nid, out in entry["outputs"].items():
            if "images" in out:
                for img in out["images"]:
                    download_image(img, os.path.join(OUTPUT_DIR, "testC_basic_txt2img.png"))
                    print("  Saved: testC_basic_txt2img.png")
    except Exception as e:
        print(f"  ERROR: {e}")
