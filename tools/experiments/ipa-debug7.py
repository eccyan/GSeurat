#!/usr/bin/env python3
"""
Debug round 7: Test IP-Adapter weight vs OpenPose strength combinations.
Goal: find the sweet spot where side-view pose is respected while keeping character identity.

Tests (all use concept_right.png + corrected 14-kp run_right f0 pose):
  A: baseline   — IPA 0.7, OpenPose 0.8 (current defaults)
  B: lower IPA  — IPA 0.4, OpenPose 0.8
  C: higher OP  — IPA 0.7, OpenPose 1.0
  D: both       — IPA 0.4, OpenPose 1.0
  E: early IPA  — IPA 0.7 end_at=0.5, OpenPose 0.8
  F: sweet spot  — IPA 0.5, OpenPose 1.0, end_at=0.7
"""
...
OUTPUT_DIR = "/Users/eccyan/dev/vulkan-game/tools/experiments/ipa-debug7-results"
...
def render_pose(keypoints, width=512, height=512):
    """Render 14-keypoint OpenPose skeleton on opaque black background."""
...
# RUN_RIGHT frame 0 — corrected 14-keypoint with T-bar shoulders
RUN_RIGHT_F0 = [
...
PROMPT = "facing right, right side profile, looking right, male knight, mid-age, no helmet, exposed face, brown short hair, plate armor, run pose, left foot extended, solo, single character, full body, centered, plain white background, solid color background"
NEGATIVE = "blurry, smooth, shadow, 3d render, photograph, photorealistic, abstract painting, multiple characters, busy background, watermark, text, signature, noise, extra limbs, deformed, partial body, cropped, helmet, visor, headgear"
SEED = 42

def build_workflow(concept_name, pose_name, ipa_weight, ipa_end_at, openpose_strength, seed):
    return {
        ...
        "42": {"class_type": "IPAdapterAdvanced", "inputs": {
            ...
            "start_at": 0.0, "end_at": ipa_end_at, "embeds_scaling": "V only",
        ...
        "47": {"class_type": "ControlNetApplyAdvanced", "inputs": {
            ...
            "strength": openpose_strength, "start_percent": 0.0, "end_percent": 1.0}},
        ...
        "9": {"class_type": "SaveImage", "inputs": {"filename_prefix": "debug7", "images": ["8", 0]}},
    }

TESTS = [
    ("A_baseline_ipa07_op08",    0.7, 1.0, 0.8),
    ("B_low_ipa04_op08",         0.4, 1.0, 0.8),
    ("C_high_op_ipa07_op10",     0.7, 1.0, 1.0),
    ("D_low_ipa04_op10",         0.4, 1.0, 1.0),
    ("E_early_ipa07_end05_op08", 0.7, 0.5, 0.8),
    ("F_sweet_ipa05_end07_op10", 0.5, 0.7, 1.0),
]

def run_test(name, concept_name, pose_name, ipa_weight, ipa_end_at, openpose_strength):
    print(f"\n--- {name} (IPA w={ipa_weight} end={ipa_end_at}, OP s={openpose_strength}) ---")
    try:
        workflow = build_workflow(concept_name, pose_name, ipa_weight, ipa_end_at, openpose_strength, SEED)
...
def main():
    ...
    pose_png = render_pose(RUN_RIGHT_F0)
    pose_name = upload_bytes(pose_png, "debug7_pose.png")

    # Save pose image for reference
    with open(os.path.join(OUTPUT_DIR, "pose_f0.png"), "wb") as f:
        f.write(pose_png)
    print(f"Saved pose image to {OUTPUT_DIR}/pose_f0.png")

    for name, ipa_weight, ipa_end_at, openpose_strength in TESTS:
        run_test(name, concept_name, pose_name, ipa_weight, ipa_end_at, openpose_strength)
